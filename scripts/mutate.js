...
const MUTATIONS = [
    ...
    { name: 'expenses: the bot loses its only view of expense rows',
      ...
      find: '    find_expenses: {\n        kind: \'read\',',
      to:   '    find_expenses: {\n        kind: \'write\',' },
    { name: 'expenses: vendor is matched exactly, so "Santiago" misses "Santiago Welding"',
      file: 'helpers/tools.js', suites: ['yard-ask-tools'],
      find: "                if (p.vendor && !like(e.vendor, p.vendor)) return false;",
      to:   "                if (p.vendor && String(e.vendor || '') !== p.vendor) return false;" },
    { name: 'expenses: notes stop being searched, so "what for" is unfindable',
      file: 'helpers/tools.js', suites: ['yard-ask-tools'],
      find: "                if (p.text && !(like(e.description, p.text) || like(e.notes, p.text))) return false;",
      to:   "                if (p.text && !like(e.description, p.text)) return false;" },
