...
const MUTATIONS = [
    ...
    { name: 'exp-web: the website loses the Expenses tab again',
      file: 'dashboard/index.html', suites: ['form-parity', 'expenses-web'],
      find: "    if (tab === 'expenses') return renderExpensesTab();\n",
      to:   '' },
    { name: 'exp-web: the modal goes back inside another tab\'s template',
      file: 'dashboard/index.html', suites: ['form-parity', 'expenses-web'],
      find: "  document.body.insertAdjacentHTML('beforeend', expenseModalHtml());",
      to:   '' },
    { name: 'exp-web: closing leaves the modal in the page, so they stack',
      file: 'dashboard/index.html', suites: ['form-parity', 'expenses-web'],
      find: "  const el = $('expenseModal');\n  if (el) el.remove();",
      to:   "  const el = $('expenseModal');\n  if (el) el.style.display = 'none';" },
    { name: 'exp-web: categories are hardcoded instead of read from the server',
      file: 'dashboard/index.html', suites: ['form-parity', 'expenses-web'],
      find: '  expenseCategories = data.categories || [];',
      to:   "  expenseCategories = ['Fuel', 'Repairs', 'Other'];" },
    { name: 'exp-web: the default method reaches the edit path too',
      file: 'dashboard/index.html', suites: ['form-parity', 'expenses-web'],
      find: "    : (expenseDefaultMethod || '');",
      to:   "    : (expenseDefaultMethod || '');\n  if (!sel.value) sel.value = expenseDefaultMethod || '';" },
    { name: 'exp-web: a retired method is no longer kept on an old expense',
      file: 'dashboard/index.html', suites: ['form-parity', 'expenses-web'],
      find: "    .concat(retired ? [`<option value=\"${esc(retired)}\">${esc(retired)} (no longer offered)</option>`] : [])",
      to:   '    .concat([])' },
    { name: 'exp-web: a short petty-cash box is no longer mentioned',
      file: 'dashboard/index.html', suites: ['form-parity', 'expenses-web'],
      find: '      if (saved && saved.expense && saved.expense.cash_shortfall) {',
      to:   '      if (false) {' },

    { name: 'expenses: the bot loses its only view of expense rows',
      file: 'helpers/tools.js', suites: ['yard-ask-tools'],
      find: '    find_expenses: {\n        kind: \'read\',',
      to:   '    find_expenses: {\n        kind: \'write\',' },
    { name: 'vendor: an INVENTED name is accepted, filing money to a stranger',
      file: 'helpers/vendorFromText.js', suites: ['yard-ask-tools'],
      // Find string repaired 2026-09-15 when the category half was added and
      // these early returns changed shape. A mutation whose pattern no longer
      // matches tests nothing and reports as a survivor.
      find: "        return cat ? { suggest: true, kind: 'category', ...cat } : { suggest: false, why: 'not_in_text' };",
      to:   "        return cat ? { suggest: true, kind: 'category', ...cat } : { suggest: true, kind: 'vendor', vendor: name, known: false, question: 'x' };" },
    { name: 'vendor: a low-confidence guess is offered as if it were sure',
      file: 'helpers/vendorFromText.js', suites: ['yard-ask-tools'],
      find: "        return cat ? { suggest: true, kind: 'category', ...cat } : { suggest: false, why: 'unsure' };",
      to:   "        return cat ? { suggest: true, kind: 'category', ...cat } : { suggest: true, kind: 'vendor', vendor: name, known: false, question: 'x' };" },
    { name: 'vendor: it second-guesses a vendor she already typed',
      file: 'helpers/vendorFromText.js', suites: ['yard-ask-tools'],
      find: "    if (String(vendor || '').trim()) return { suggest: false, why: 'vendor_already_set' };",
      to:   '' },
    { name: 'vendor: her own spelling is not preferred over the model\'s',
      file: 'helpers/vendorFromText.js', suites: ['yard-ask-tools'],
      find: '        vendor: match || name,',
      to:   '        vendor: name,' },
    { name: 'vendor: a name is matched fuzzily against the ones on file',
      file: 'helpers/vendorFromText.js', suites: ['yard-ask-tools'],
      find: '    const match = known.find((k) => norm(k) === norm(name)) || null;',
      to:   '    const match = known.find((k) => norm(k).includes(norm(name).slice(0, 3))) || null;' },
    { name: 'vendor: a hung model hangs the form instead of giving up',
      file: 'helpers/vendorFromText.js', suites: ['yard-ask-tools'],
      find: '    const out = await withTimeout(Promise.resolve().then(\n        () => call(buildPrompt(text, known, CATEGORIES, category))), TIMEOUT_MS);',
      to:   '    const out = await Promise.resolve().then(() => call(buildPrompt(text, known, CATEGORIES, category))).catch(() => null);' },
    { name: 'vendor: an ambiguous line is answered instead of asked about',
      file: 'helpers/vendorFromText.js', suites: ['yard-ask-tools'],
      find: '    const cat = ambiguous.length >= 2',
      to:   '    const cat = false' },
    { name: 'vendor: a category the model invented is accepted',
      file: 'helpers/vendorFromText.js', suites: ['yard-ask-tools'],
      find: '    const inList = (c) => CATEGORIES.find((k) => norm(k) === norm(c)) || null;',
      to:   '    const inList = (c) => (c ? String(c) : null);' },
    { name: 'vendor-ui: a suggested category is set on a select with no such option',
      file: 'dashboard/index.html', suites: ['expenses-web'],
      find: "      if (![...sel.options].some((o) => o.value === c)) {\n        sel.insertAdjacentHTML('afterbegin', `<option value=\"${esc(c)}\">${esc(c)}</option>`);\n      }",
      to:   '' },
    { name: 'vendor-ui: both questions are shown at once',
      file: 'dashboard/index.html', suites: ['expenses-web'],
      find: "      if (r.kind === 'vendor' && r.category_hint) {",
      to:   '      if (false) {' },
    { name: 'vendor-ui: the form fills the vendor in without asking',
      file: 'dashboard/index.html', suites: ['expenses-web'],
      // Repaired 2026-09-15: the buttons became class-based when the category
      // question was added, so the old pattern stopped matching.
      find: "    if (yes) yes.addEventListener('click', () => {",
      to:   "    $('exp_vendor').value = r.vendor;\n    if (yes) yes.addEventListener('click', () => {" },
    { name: 'vendor-ui: the same line is asked about again and again',
      file: 'dashboard/index.html', suites: ['expenses-web'],
      find: '    if (!text || text === vendorAskedFor) return;',
      to:   '    if (!text) return;' },
    { name: 'vendor-ui: it asks even when she typed a vendor herself',
      file: 'dashboard/index.html', suites: ['expenses-web'],
      find: "    if ($('exp_vendor').value.trim()) { hide(); return; }",
      to:   '' },

    { name: 'expenses: the spend summary loses its category breakdown',
      file: 'helpers/tools.js', suites: ['yard-ask-tools'],
      find: "                byCategory = (getExpenseReport(loadExpenses(), { from: p.from, to: p.to }).byCategory) || [];",
      to:   '                byCategory = [];' },
    { name: 'expenses: a name is matched exactly, so "Santiago" misses "Santiago Welding"',
      file: 'helpers/tools.js', suites: ['yard-ask-tools'],
      find: "                if (needle && !(like(e.vendor, needle) || like(e.description, needle) || like(e.notes, needle))) return false;",
      to:   "                if (needle && String(e.vendor || '') !== needle) return false;" },
    { name: 'expenses: only the vendor field is searched — the undercount returns',
      file: 'helpers/tools.js', suites: ['yard-ask-tools'],
      find: "                if (needle && !(like(e.vendor, needle) || like(e.description, needle) || like(e.notes, needle))) return false;",
      to:   "                if (needle && !like(e.vendor, needle)) return false;" },
    { name: 'expenses: notes stop being searched, so "what for" is unfindable',
      file: 'helpers/tools.js', suites: ['yard-ask-tools'],
      find: "                if (needle && !(like(e.vendor, needle) || like(e.description, needle) || like(e.notes, needle))) return false;",
      to:   "                if (needle && !(like(e.vendor, needle) || like(e.description, needle))) return false;" },
    { name: 'expenses: the old vendor/text aliases stop working',
      file: 'helpers/tools.js', suites: ['yard-ask-tools'],
      find: "            const needle = p.match || p.vendor || p.text || '';",
      to:   "            const needle = p.match || '';" },
    { name: 'expenses: the total is left for the model to add up',
      file: 'helpers/tools.js', suites: ['yard-ask-tools'],
      find: "            return { matched_total: total, ...cap(rows) };",
      to:   '            return { ...cap(rows) };' },
    { name: 'expenses: the date range is ignored',
      file: 'helpers/tools.js', suites: ['yard-ask-tools'],
      find: "                if (p.from && String(e.date || '') < p.from) return false;",
      to:   '' },

    { name: 'bot: the catch goes back to one sentence for every failure',
      file: 'dashboard/yard-bot.js', suites: ['form-parity'],
      find: "        if (status === 403 || /staff access is limited/i.test(msg)) {",
      to:   '        if (false) {' },
    { name: 'bot: callApi swallows the status, so a 403 looks like an outage',
      file: 'dashboard/yard-bot.js', suites: ['form-parity'],
      find: '          err.status = r.status;',
      to:   '' },
    { name: 'bot: the status is no longer recovered from the message text',
      file: 'dashboard/yard-bot.js', suites: ['form-parity'],
      find: "            || Number((msg.match(/request failed \\((\\d{3})\\)/) || [])[1])\n",
      to:   '' },
    { name: 'bot: a 500 is reported as unreachable again',
      file: 'dashboard/yard-bot.js', suites: ['form-parity'],
      find: "        } else if (status >= 500) {",
      to:   '        } else if (false) {' },
    { name: 'bot: /api/yard/ask is quietly added to the staff allowlist',
      file: 'api.js', suites: ['form-parity'],
      find: "'/api/trucker-bills', '/api/verify-admin-password'];",
      to:   "'/api/trucker-bills', '/api/verify-admin-password', '/api/yard'];" },

    { name: 'kind: an untagged old load counts as a sale, not a purchase (web)',
      file: 'dashboard/index.html', suites: ['load-card-actions'],
      find: "  return (loads || []).filter((l) => (l._kind === 'sale' ? 'sale' : 'purchase') === kind);",
      to:   "  return (loads || []).filter((l) => (l._kind === 'purchase' ? 'purchase' : 'sale') === kind);" },
    { name: 'kind: the same, on the app',
      file: 'mobile-app/www/index.html', suites: ['load-card-actions'],
      find: "  return (loads || []).filter((l) => (l._kind === 'sale' ? 'sale' : 'purchase') === kind);",
      to:   "  return (loads || []).filter((l) => (l._kind === 'purchase' ? 'purchase' : 'sale') === kind);" },
    { name: 'kind: the filter is ignored, so every tab shows everything (web)',
      file: 'dashboard/index.html', suites: ['load-card-actions'],
      find: '    let rows = filterLoadsByKind(loadsCache, loadKindFilter);',
      to:   '    let rows = loadsCache;' },
    { name: 'kind: Sales-only still renders grouped by date (web)',
      file: 'dashboard/index.html', suites: ['load-card-actions'],
      find: "    const narrowed = !!q || !!chosenItemDetail || loadKindFilter !== 'all';",
      to:   '    const narrowed = !!q || !!chosenItemDetail;' },
    { name: 'kind: handlers bound to the buttons, so the bar works once (web)',
      file: 'dashboard/index.html', suites: ['load-card-actions'],
      find: "  if (kindBar) kindBar.addEventListener('click', (ev) => {",
      to:   "  if (kindBar) kindBar.querySelector('x').addEventListener('click', (ev) => {" },
    { name: 'kind: the choice is forgotten between visits (web)',
      file: 'dashboard/index.html', suites: ['load-card-actions'],
      find: "    try { localStorage.setItem('loadKindFilter', loadKindFilter); } catch (e) {}",
      to:   '' },

    { name: 'app: the Pay button on a sale loses its direction (mobile)',
      file: 'mobile-app/www/index.html', suites: ['form-parity'],
      find: "${l._kind === 'sale' ? 'Receive payment' : 'Pay'}",
      to:   'Pay' },
    { name: 'app: the pay modal keeps one title for both directions (mobile)',
      file: 'mobile-app/www/index.html', suites: ['form-parity'],
      find: "if (ttl) ttl.textContent = sale ? 'Receive payment' : 'Record payment';",
      to:   "if (ttl) ttl.textContent = 'Record payment';" },
    { name: 'app: a sale reports money already PAID, not received (mobile)',
      file: 'mobile-app/www/index.html', suites: ['form-parity'],
      find: "already ${sale ? 'received' : 'paid'}",
      to:   'already paid' },
    { name: 'web: the Pay button on a sale loses its direction (dashboard)',
      file: 'dashboard/index.html', suites: ['form-parity'],
      find: "${l._kind === 'sale' ? 'Receive payment' : 'Pay'}",
      to:   'Pay' },
    { name: 'web: the pay modal heading loses the id it is written into',
      file: 'dashboard/index.html', suites: ['form-parity'],
      find: '<div id="payModalTitle" style="font-weight:700; text-transform:uppercase; font-size:14px;">Record payment</div>',
      to:   '<div style="font-weight:700; text-transform:uppercase; font-size:14px;">Record payment</div>' },
    ...
    // ── "SEND IT" AT AN EMAIL CONFIRM, 2026-09-10 ────────────────────────
    { name: '"send it" falls through to the AI again',
      file: 'workflow/brain.js', suites: ['send-it-is-yes'],
      find: '            if (bare.test(ctx.text)) {',
      to:   '            if (false) {' },
    { name: '"send it to somebody else" silently sends to the addressee',
      file: 'workflow/brain.js', suites: ['send-it-is-yes'],
      find: '                if (isSame) {',
      to:   '                if (true) {' },
    { name: 'send-means-yes leaks to every other open question',
      file: 'workflow/brain.js', suites: ['send-it-is-yes'],
      find: "        if (p.type === 'await_email_confirm') {\n            const bare =",
      to:   '        if (true) {\n            const bare =' },

    { name: 'a container claimed by two bills is not reported',
      file: 'helpers/bills.js', suites: ['bills-sales'],
      find: '    return [...seen.values()].filter((d) => d.ids.length > 1);\n}\n\nfunction groupColumns(groupId) {',
      to:   '    return [];\n}\n\nfunction groupColumns(groupId) {' },
    { name: 'the margin silently keeps the last bill for a container',
      file: 'helpers/margin.js', suites: ['bills-sales'],
      find: '        if (row.bill_id) {\n            row.duplicate_bill_ids = (row.duplicate_bill_ids || [row.bill_id]).concat(b.id);\n        }',
      to:   '' },
    { name: 'the duplicate count is missing from the margin summary',
      file: 'helpers/margin.js', suites: ['bills-sales'],
      find: "        conflicted: r.filter((x) => (x.duplicate_bill_ids || []).length\n                                 || (x.duplicate_sale_ids || []).length).length,",
      to:   '        conflicted: 0,' },
    { name: 'the bills route stops telling the client about duplicates',
      file: 'api.js', suites: ['bills-sales'],
      find: '                duplicates: b.duplicates(all),',
      to:   '                duplicates: [],' },

    // ── THE SHIPMENT SHEET, 2026-09-10 ───────────────────────────────────
    { name: 'a drifted header is left in place, misaligning every row',
      file: 'helpers/shipmentSheetLog.js', suites: ['bills-sales'],
      find: '    if (same) return { changed: false };',
      to:   '    if (true) return { changed: false };' },
    { name: 'the header is rewritten but the rows under it are not remapped',
      file: 'helpers/shipmentSheetLog.js', suites: ['bills-sales'],
      find: '        const i = oldIndex.get(String(label).trim());\n        return i === undefined ? \'\' : (row[i] === undefined ? \'\' : row[i]);',
      to:   '        return row[wanted.indexOf(label)] === undefined ? \'\' : row[wanted.indexOf(label)];' },
    { name: 'a multi-grade container collapses back to one sheet row',
      file: 'helpers/shipmentSheetLog.js', suites: ['bills-sales'],
      find: '    const items = Array.isArray(b.items) ? b.items : [];',
      to:   '    const items = [];' },
    { name: 'the sheet key drops the grade, so a second item overwrites the first',
      file: 'helpers/shipmentSheetLog.js', suites: ['bills-sales'],
      find: '        const key = `${b.id}:${it.id}`;',
      to:   '        const key = `${b.id}`;' },
    { name: 'trucking is repeated on every grade of a container',
      file: 'helpers/shipmentSheetLog.js', suites: ['bills-sales'],
      find: "            trucking_amount: '', net_payable: '', balance: '',",
      to:   '' },
    { name: 'the split editor goes back onto the bill form',
      file: 'dashboard/index.html', suites: ['ledger-render'],
      find: "          ? `<div style=\"font-size:11px; color:${LEDGER_LIGHT.inkFaint}; margin-top:8px;\">Line Haul, Port Fees and the rest are edited under Trucking — click the haul there.</div>`",
      to:   "          ? '<div id=\"ledSplitBox\"></div>'" },
    { name: 'the item lines lose their MT figure again',
      file: 'dashboard/index.html', suites: ['ledger-render'],
      find: '          <span class="ledItemMt" data-i="${i}"',
      to:   '          <span class="ledItemMt" data-x="${i}"' },
    ...
    { name: 'the invoice line amount goes back to unrounded floating point',
      ...
      find: '            : round2(qty * rate);',
      to:   '            : Number(qty * rate);' },
    { name: 'the invoice stops fitting to one page again',
      file: 'helpers/invoicePdf.js', suites: ['pdf-one-page'],
      find: '            const pdf = await pdfFittedToOnePage(page, {',
      to:   '            const pdf = await page.pdf({' },
    { name: 'the invoice signature goes back to a second inline copy',
      file: 'helpers/invoicePdf.js', suites: ['signature'],
      find: "        signature_src: require('./signature').signatureDataUrl() || '',",
      to:   "        signature_src: ''," },
    ...
    // ── the delivery-status enquiry (2026-09-10) ────────────────────────
    // Every one of these is a way the enquiry goes wrong WITHOUT raising
    // anything: a message to the wrong haulier, two messages an hour apart,
    // one about a load that arrived yesterday, or one carrying the buyer's
    // price. None of them would show up as an error in her log.
    { name: 'delivery: the trucker is told what the buyer paid',
      file: 'helpers/deliveryEnquiry.js', suites: ['delivery-enquiry'],
      find: '    if (net) bits.push(`${net} ${unit}`);',
      to:   '    if (net) bits.push(`${net} ${unit}`);\n    if (load.amount) bits.push(`$${load.amount}`);' },
    { name: 'delivery: a trucker with no number, group or email is scheduled anyway',
      file: 'helpers/deliveryEnquiry.js', suites: ['delivery-enquiry'],
      find: "    const reachable = !!(hit.group_id || hit.whatsapp\n        || (hit.preferred_mode === 'email' && hit.email));",
      to:   '    const reachable = true;' },
    { name: 'delivery: two roster entries collapsing to one name is guessed between',
      file: 'helpers/deliveryEnquiry.js', suites: ['delivery-enquiry'],
      find: '        if (near.length === 1) hit = near[0];',
      to:   '        if (near.length >= 1) hit = near[0];' },
    { name: 'delivery: an ETA that has already gone fires the instant she saves',
      file: 'helpers/deliveryEnquiry.js', suites: ['delivery-enquiry'],
      find: '    if (at.getTime() <= now.getTime()) {',
      to:   '    if (false) {' },
    { name: 'delivery: an enquiry is scheduled on a load already delivered',
      file: 'helpers/deliveryEnquiry.js', suites: ['delivery-enquiry'],
      find: '    if (isDelivered(load)) {',
      to:   '    if (false) {' },
    { name: 'delivery: moving the ETA leaves the old enquiry in place too',
      file: 'helpers/deliveryEnquiry.js', suites: ['delivery-enquiry'],
      find: "    const cancelled = await cancelForLoad(load && load.id, opts.reason || 'eta_changed');",
      to:   '    const cancelled = 0;' },
    { name: 'delivery: the gate stops noticing she marked it delivered',
      file: 'helpers/tasks.js', suites: ['delivery-enquiry'],
      find: "        if (String(load.delivery_status || '').toLowerCase() === 'delivered') return 'skip';",
      to:   '' },
    { name: 'delivery: a stale task pinned to the OLD ETA fires anyway',
      file: 'helpers/tasks.js', suites: ['delivery-enquiry'],
      find: "            if (!now || now.toISOString() !== task.condition.eta_at) return 'skip';",
      to:   '' },
    { name: 'delivery: a recycled load id inherits the deleted load\'s enquiry',
      file: 'helpers/tasks.js', suites: ['delivery-enquiry'],
      find: "        if (task.condition.load_created_at\n            && load.created_at !== task.condition.load_created_at) return 'skip';",
      to:   '' },
    { name: 'delivery: an edit quietly puts a delivered load back in transit',
    ...
    // ── the per-supplier running account (2026-09-11) ───────────────────
    // A running balance is the worst thing to get wrong quietly: every row
    // still looks right, and only the closing figure is a lie.
    { name: 'account: an applied advance is credited a SECOND time',
      file: 'helpers/supplierAccount.js', suites: ['supplier-account'],
      find: '            credit: round2(num(p.amount)),',
      to:   '            credit: round2(num(p.amount) + (p.allocations || []).reduce((s, a) => s + (num(a.amount) || 0), 0)),' },
    { name: 'account: the debit is the bill NET, so haulage vanishes from what she owes',
      file: 'helpers/supplierAccount.js', suites: ['supplier-account'],
      find: '        const debit = round2(num(computed.amount));',
      to:   '        const debit = round2(num(computed.net_payable));' },
    { name: 'account: an advance puts its amount in the debit column too',
      file: 'helpers/supplierAccount.js', suites: ['supplier-account'],
      find: '            debit: null,\n            credit: round2(num(p.amount)),',
      to:   '            debit: round2(num(p.amount)),\n            credit: round2(num(p.amount)),' },
    // NOT mutated to `[row.iso]` alone: that is an EQUIVALENT mutant. V8's
    // sort is stable and the bills loop pushes before the payments loop, so
    // dropping the tiebreaker changes nothing TODAY — it only removes the
    // guarantee. What is mutated is the precedence itself, which is a
    // difference she can see in the balance column.
    { name: 'account: net weight is read from net_lb only, blank on most new bills',
      file: 'helpers/supplierAccount.js', suites: ['supplier-account'],
      find: '    const lb = num(computed.net_lb) !== null ? num(computed.net_lb) : num(computed.items_weight);',
      to:   '    const lb = num(computed.net_lb);' },
    { name: 'account: a per-MT price is shown beside a weight in pounds',
      file: 'helpers/supplierAccount.js', suites: ['supplier-account'],
      find: "        : (unit === 'mt' ? Math.round((lb / bills.LB_PER_MT) * 1000) / 1000 : lb);",
      to:   '        : lb;' },
    { name: 'account: two grades at two rates get a blended price invented for them',
      file: 'helpers/supplierAccount.js', suites: ['supplier-account'],
      find: '            mixed = true;',
      to:   '            mixed = false; price = distinct[0];' },
    { name: 'account: a same-day payment is ranked before the bill it pays',
      file: 'helpers/supplierAccount.js', suites: ['supplier-account'],
      find: "row.kind === 'bill' ? '0' : '1'",
      to:   "row.kind === 'bill' ? '1' : '0'" },
    { name: 'account: a filtered statement restarts the balance at zero',
      file: 'helpers/supplierAccount.js', suites: ['supplier-account'],
      find: '    let running = opening;',
      to:   '    let running = 0;' },
    { name: 'account: every supplier\'s payments land on every account',
      file: 'helpers/supplierAccount.js', suites: ['supplier-account'],
      find: '        if (!sameSupplier(p.supplier, who)) continue;',
      to:   '' },
    { name: 'account: a bill with no supplier is silently dropped',
      file: 'helpers/supplierAccount.js', suites: ['supplier-account'],
      find: "        .filter((b) => !String(b.supplier || '').trim())",
      to:   '        .filter(() => false)' },
    { name: 'account: the closing figure is re-summed instead of read off the last row',
      file: 'helpers/supplierAccount.js', suites: ['supplier-account'],
      find: '        closing: list && list.length ? list[list.length - 1].balance : round2(opening) || 0,',
      to:   '        closing: round2(debit - credit) || 0,' },
    { name: 'inventory: a receipt with no supplier is accepted',
      file: 'helpers/edgeInventory.js', suites: ['supplier-account'],
      find: "    if (!supplier) throw new Error('Validation: which supplier delivered this?');",
      to:   '' },
    { name: 'account: a delivery is debited, counting the purchase twice',
      file: 'helpers/supplierAccount.js', suites: ['supplier-account'],
      find: '    for (const p of billPayments.list()) {',
      to:   "    for (const r of require('./edgeInventory').forSupplier(who)) {\n"
          + "        out.push({ id: r.id, kind: 'receipt', date: r.date, iso: bills.sortableDate(r.date) || '',\n"
          + "                   what: 'Delivery', debit: r.amount, credit: null, created_at: r.created_at });\n"
          + '    }\n    for (const p of billPayments.list()) {' },
    { name: 'inventory: grades are totalled across every supplier',
      file: 'helpers/edgeInventory.js', suites: ['supplier-account'],
      find: '    for (const r of forSupplier(supplier)) {',
      to:   '    for (const r of list()) {' },

    { name: 'storage: buildRecord drops the yard, so an edit loses it',
      file: 'helpers/edgeInventory.js', suites: ['supplier-account'],
      find: "        storage: String(input.storage || '').trim() || null,",
      to:   '        storage: null,' },
    { name: 'storage: a blank yard is folded in with a real one',
      file: 'helpers/edgeInventory.js', suites: ['supplier-account'],
      find: "        const key = r.storage ? String(r.storage).trim().toLowerCase().replace(/\\s+/g, ' ') : '';",
      to:   "        const key = String(r.storage || '').trim().toLowerCase();" },
    { name: 'storage: two spellings of one yard become two yards',
      file: 'helpers/edgeInventory.js', suites: ['supplier-account'],
      find: "    === String(b || '').trim().toLowerCase().replace(/\\s+/g, ' ');",
      to:   "    === String(b || '').trim();" },
    { name: 'storage: the yard is labelled with the latest spelling, not the earliest',
      file: 'helpers/edgeInventory.js', suites: ['supplier-account'],
      find: '    const byAge = [...rows].sort((x, y) =>\n        String(x.created_at || \'\').localeCompare(String(y.created_at || \'\')));\n\n    for (const r of byAge) {',
      to:   '    for (const r of rows) {' },
    { name: 'storage: the unrecorded bucket sorts by weight like a real yard',
      file: 'helpers/edgeInventory.js', suites: ['supplier-account'],
      find: '            if (a.recorded !== b.recorded) return a.recorded ? -1 : 1;\n',
      to:   '' },
    { name: 'storage: filtering to a yard ignores the filter',
      file: 'helpers/edgeInventory.js', suites: ['supplier-account'],
      find: '            return sameYard(r.storage, yard);',
      to:   '            return true;' },

    { name: 'storage: a supplier who has delivered but not been billed vanishes',
      file: 'helpers/supplierAccount.js', suites: ['supplier-account'],
      find: "    for (const s of inv.suppliers()) if (!seen.has(s.toLowerCase())) seen.set(s.toLowerCase(), s);\n",
      to:   '' },
    { name: 'page: the By yard tab shows a bare label with no warning on the gap',
      file: 'dashboard/edge-inventory.html', suites: ['supplier-account'],
      find: "${y.recorded ? '' : ' &mdash; nobody said where this went'}",
      to:   '' },
    { name: 'page: the yard filter drops the way to find unrecorded rows',
      file: 'dashboard/edge-inventory.html', suites: ['supplier-account'],
      find: "    .concat('<option value=\"(not recorded)\">(not recorded)</option>').join('');",
      to:   "    .join('');" },
    { name: 'page: the delivery form stops sending the yard',
      file: 'dashboard/edge-inventory.html', suites: ['supplier-account'],
      find: "    storage: $('rm_storage').value.trim(),\n",
      to:   '' },

    { name: 'page: Price is rendered after Debit, not before it',
      file: 'dashboard/edge-inventory.html', suites: ['supplier-account'],
      find: '<th style="text-align:right;">Net weight</th><th style="text-align:right;">Price</th>\n        <th style="text-align:right;">Debit</th>',
      to:   '<th style="text-align:right;">Debit</th>\n        <th style="text-align:right;">Net weight</th><th style="text-align:right;">Price</th>' },
    { name: 'page: a blank figure renders as an em dash, reading as zero',
      file: 'dashboard/edge-inventory.html', suites: ['supplier-account'],
      find: "const money = (n) => (n === null || n === undefined || n === '') ? ''",
      to:   "const money = (n) => (n === null || n === undefined || n === '') ? '\u2014'" },
    { name: 'page: the delivery form opens with no packing-list line',
      file: 'dashboard/edge-inventory.html', suites: ['supplier-account'],
      find: '  recItems = [{}]; paintRecItems();',
      to:   '  recItems = []; paintRecItems();' },
    ...
    { name: 'items: the header gets its own column widths again',
      file: 'dashboard/index.html', suites: ['ledger-render'],
      find: '<div class="ledItemsHead" style="${ITEM_GRID}',
      to:   '<div class="ledItemsHead" style="display:grid; grid-template-columns:minmax(0,1fr) 86px 66px 72px 88px 78px; gap:8px; align-items:center;' },
    { name: 'items: the totals row drifts from the rows above it',
      file: 'dashboard/index.html', suites: ['ledger-render'],
      find: '<div class="ledItemsFoot" style="${ITEM_GRID}',
      to:   '<div class="ledItemsFoot" style="display:flex; align-items:center; gap:8px;' },
    { name: 'items: the description input gets a second style attribute again',
      file: 'dashboard/index.html', suites: ['ledger-render'],
      find: `<input data-i="\${i}" data-f="description" list="ledlist-description" \${cell(it.description, 'grade', '100%')}>`,
      to:   `<input data-i="\${i}" data-f="description" list="ledlist-description" \${cell(it.description, 'grade', '100%')} style="flex:1;">` },
    { name: 'items: a header column is dropped, so everything after it shifts',
      file: 'dashboard/index.html', suites: ['ledger-render'],
      find: '          <span style="text-align:right;">Net MT</span>\n',
      to:   '' },
    { name: 'items: the price box goes back to left-aligned prose',
      file: 'dashboard/index.html', suites: ['ledger-render'],
      find: `<input data-i="\${i}" data-f="price" \${cell(it.price, 'price', '100%', 'text-align:right; font-family:var(--font-mono);')}>`,
      to:   `<input data-i="\${i}" data-f="price" \${cell(it.price, 'price', '100%')}>` },

    { name: 'delivery: buildRecord drops the ETA, so an edit un-schedules the chase',
      file: 'helpers/outboundLoads.js', suites: ['delivery-enquiry'],
