...
const MUTATIONS = [
    ...
    { name: 'vendor: an INVENTED name is accepted, filing money to a stranger',
      ...
      find: "    if (!norm(text).includes(norm(name))) return { suggest: false, why: 'not_in_text' };",
      to:   '' },
    { name: 'vendor: a low-confidence guess is offered as if it were sure',
      file: 'helpers/vendorFromText.js', suites: ['yard-ask-tools'],
      find: "    if (out.confidence && out.confidence !== 'high') return { suggest: false, why: 'unsure' };",
      to:   '' },
    ...
    { name: 'vendor: a hung model hangs the form instead of giving up',
      file: 'helpers/vendorFromText.js', suites: ['yard-ask-tools'],
      find: '    const out = await withTimeout(Promise.resolve().then(() => call(buildPrompt(text, known))), TIMEOUT_MS);',
      to:   '    const out = await Promise.resolve().then(() => call(buildPrompt(text, known))).catch(() => null);' },
    { name: 'vendor-ui: the form fills the vendor in without asking',
      file: 'dashboard/index.html', suites: ['expenses-web'],
      find: "    $('expVendorYes').addEventListener('click', () => {\n      $('exp_vendor').value = r.vendor;",
      to:   "    $('exp_vendor').value = r.vendor;\n    $('expVendorYes').addEventListener('click', () => {" },
