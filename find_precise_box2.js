...
(async () => {
  ...
  for (let y=0;y<h;y++) for(let x=0;x<w;x++){
    ...
    if (r>140 && (r-g)>60 && (r-b)>60) { if (x<minX)minX=x; if(x>maxX)maxX=x; if(y<minY)minY=y; if(y>maxY)maxY=y; }
  }

  const meta = await sharp(buf).rotate().metadata();
  const W = meta.width, H = meta.height;
  const marginX = (maxX-minX)/w * 0.1, marginY = (maxY-minY)/h * 0.15;
  ...
  console.log('crop region (orig px)', left, top, width, height, 'of', W, H);

  const cropped = await sharp(buf).rotate().extract({left, top, width, height}).resize({width: 2000, withoutEnlargement:false}).jpeg({quality:95}).toBuffer();
  ...
  const text = await visionOcr.detectText(cropped.toString('base64'));
  console.log('Vision TEXT_DETECTION on precise crop:', JSON.stringify(text));
})();
