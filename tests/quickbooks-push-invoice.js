...
ck('priced per MT -> Qty in MT (Jarvis and QuickBooks agree on this half-cent: both give 13923.62)', r.invoice.Line[0].SalesItemLineDetail.Qty === 21.587 && r.invoice.Line[0].SalesItemLineDetail.UnitPrice === 645 && r.invoice.Line[0].Amount === 13923.62);
...
const half = buildInvoice([{ ...rows[0], weight: 13.277, invoice_price: 645.05, amount: 8563.99, receivable: 8563.99 }], refs);
ck('half-cent line (8,563.99) carries the amount alone — QuickBooks rounds the half-cent up, JS down', half.invoice && half.invoice.Line[0].Amount === 8563.99 && half.invoice.Line[0].SalesItemLineDetail.Qty === undefined, JSON.stringify(half.problems));
const exact = require('../helpers/quickbooks/push').pairFits;
ck('pairFits: clean cents pass', exact(21.587, 645, 13923.62) || exact(21.587, 645, 13923.61));
ck('pairFits: half-cent product refused', exact(13.277, 645.05, 8563.99) === false && exact(13.277, 645.05, 8563.98) === false);
ck('pairFits: product right but amount different -> refused', exact(10, 2, 21) === false);
