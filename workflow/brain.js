...
function policyDecide(ctx) {
    ...
    // ── A0b2. "Generate invoice for the Inesh container" — the answer to
    // whichever question that flow asked (a pick, a missing field, the
    // invoice number, yes/no, the layout). Same shape as the container-number
    // capture: the pending is the validation. Free-text stages (customer,
    // item) carry arbitrate so a genuinely new request still escapes.
    // A bug's status only changes on an explicit yes — see actions.closeBugForConfirm.
    if (ctx.pendingAction?.type === 'await_bug_close') {
        if (/^\s*(yes|yeah|yep|yup|ok|okay|sure|do it|correct|right)\b/i.test(t)) {
            return { intent: 'bug_close_yes', resolvedBy: 'policy', data: {} };
        }
    ...
    if (ctx.pendingAction?.type === 'await_sale_invoice') {
        return { intent: 'sale_invoice_answer', resolvedBy: 'policy', arbitrate: !!ctx.pendingAction.arbitrate, data: { answer_text: ctx.text.trim() } };
    }
    ...
    if (ctx.isManagerOrTeam) {
        ...
        // resolution as "reply to N", same reasoning for checking it before
        // any general "ignore" phrasing gets near the AI — a bare number
        // must be read as a digest position, not a fresh classification.
        if ((m = ctx.text.trim().match(/^(?:please\s+)?(?:ignore|dismiss|skip|clear)\s+#?(\d{1,2}(?:\s*(?:,|and)\s*#?\d{1,2})*)\s*$/i))) {
            const indices = m[1].split(/,|\band\b/i).map((x) => x.trim()).filter(Boolean);
            return { intent: 'ignore_digest_item', resolvedBy: 'policy', data: { indices } };
        }
        // "ignore all" / "ignore all of them" / "dismiss everything" — the
        // natural extension of the same question, thought through this time
        // rather than waiting for her to hit the gap and ask again.
        if (/^(?:please\s+)?(?:ignore|dismiss|skip|clear)\s+(?:all|everything|all of (?:it|them))\s*$/i.test(ctx.text.trim())) {
            return { intent: 'ignore_digest_item', resolvedBy: 'policy', data: { indices: null, all: true } };
        }
        ...
        // NO SENDER IS NAMED. It was previously re-expressed as a slightly
        // different lookahead on each pattern, and two of them were wrong —
        // "check my inbox for Zimex" and "show me the digest from Zimex" both
        // reached the inbox branch and would have answered a targeted question
        // with a generic dump. Stated once now, applied to every branch.
        //
        // "for any new mail" must NOT count as naming somebody, so the word
        // after the preposition is checked against the generic vocabulary she
        // actually uses when she means "everything".
        const GENERIC_AFTER_PREP = /^(?:any|new|all|my|me|the|a|an|today|todays|today's|this|that|those|these|unread|latest|pending|mails?|e?mails?|messages?|inbox|mail\s*box|repl(?:y|ies)|response|attention|action|now|it|them)\b/i;
        // EVERY preposition, not the first. "check my mailbox for mail from
        // Kristal" took the first "for", saw the generic word "mail", and
        // declared nobody named — burying a question about Kristal under the
        // whole inbox. Any one non-generic follower is enough.
        const namesSomeone = (t) => {
            const text = String(t || '').trim();
            const re = /\b(?:from|for|by)\s+(\S.*)$/gi;
            let m;
            while ((m = re.exec(text)) !== null) {
                if (!GENERIC_AFTER_PREP.test(m[1].trim())) return true;
                // Advance past this preposition so the next one is examined
                // rather than the regex restarting at the same spot.
        ...
        if (/^(?:what|which|any|anything)?\s*(?:emails?|mails?)?\s*(?:do\s+i\s+|that\s+)?(?:needs?|requires?|waiting\s+(?:on|for))\s*(?:my\s+|a\s+)?(?:reply|response|answer|attention)\b/i.test(ctx.text.trim())
            // "mailbox" was missing, and the trailing-text case with it —
            // LIVE 2026-08-29: "Check my mailbox for any new mail" fell through
            // to the AI, which summarised two random payment debits as "new
            // mail". The \b anchor stopped at the noun, so anything after it
            // missed. Trailing text is allowed now as long as no sender is
            // named — see the negative tests.
            || /^(?:check|show|list|see|give\s+me|pull\s+up|open)\s+(?:me\s+)?(?:my\s+|the\s+)?(?:inbox|mail\s*box|unreplied|pending\s+e?mails?)\b/i.test(ctx.text.trim())
            // "Email digest" — LIVE 2026-08-29, answered with "I can help with
            // that. What specifically would you like to see in the email
            // digest?" A clarifying question about the ONE thing the digest is.
            // She named the feature by its name; there is nothing to clarify.
            || /^(?:the\s+|my\s+|todays?\s+|today'?s\s+)?(?:e?mail\s+)?digest\b|^(?:show|send|give)\s+(?:me\s+)?(?:the\s+|my\s+)?(?:e?mail\s+)?digest\b/i.test(ctx.text.trim())
            // "All mails received today which needed my attentiom" (sic) —
            // LIVE 2026-08-29, searched the inbox for that whole sentence.
            // Anything of the shape "<mail words> ... need/require my
            // attention/reply/action" with no sender named.
            // "needing" was missed by need(?:ed|s)? — a live phrasing gap.
            || (/(?:e?mails?|messages?|inbox|mail\s*box)/i.test(ctx.text)
                && /need(?:ed|s|ing)?\s+my\s+(?:atten\w*|repl\w*|response|action|input|eyes)/i.test(ctx.text))
            ...
            return namesSomeone(ctx.text)
                ...
                // she asked about. Let the AI path pick search_mail.
                ? { needsAI: true, resolvedBy: 'policy' }
                : { intent: 'show_pending_replies', resolvedBy: 'policy', data: {} };
        }
        ...
        // ── "send the documents for HMMU7060866" ────────────────────────────
        // Apsara, 2026-09-17: "now once the invoice and packing list is
        // created -when i give command to jarvis to send--->it should able to
        // mail customer with documents..", naming the shipment BY CONTAINER
        // NUMBER.
        //
        // This is a narrow pattern ON PURPOSE, and it is not the treadmill the
        // note above warns against. It keys on a CONTAINER NUMBER — ISO 6346,
        // four letters and seven digits — which is an identifier with a fixed
        // shape, not a phrasing. Nothing else she says looks like one, so
        // there is no second pattern to add next week for a different way of
        // wording it. Every OTHER phrasing falls through to aiDecide, which
        // carries this intent too.
        //
        // It must sit ABOVE the "email X about Y" rule: "email eccomelt about
        // HMMU7060866" matches both, and the document send is the specific
        // one. It stages a confirmation — nothing here sends.
        // ── IT REQUIRES A DOCUMENT WORD, AND THAT IS DELIBERATE ─────────────
        // A bare "send HMMU7060866" is NOT claimed here. That form already
        // belongs to forward_booking, which has been forwarding bookings on
        // "send <number>" for months, and a booking number can be four
        // letters and seven digits just as a container can. Taking the bare
        // form would have been me changing behaviour she did not ask to
        // change, to support a phrasing I invented — hers was "send the
        // documents for HMMU7060866".
        //
        // So: a container number AND one of the document words. If she ever
        // wants the bare form too, it is a one-line change, but it is her
        // call and not mine.
        // ── "GENERATE INVOICE FOR THE INESH CONTAINER" ─────────────────
        // Apsara, 2026-09-21. The decision lives in
        // helpers/saleInvoiceFlow.saleInvoiceRequest, shared with the voice
        // route: a making verb and "invoice", no proforma/send/payment word,
        // no question. Her rule the same day: "invoice should not create
        // proforma" — a proforma is asked for by name.
        {
            let req = null;
            try { req = require('../helpers/saleInvoiceFlow').saleInvoiceRequest(ctx.text); } catch (e) { req = null; }
            if (req) {
                return { intent: 'generate_sale_invoice', resolvedBy: 'policy',
                    data: { container_no: req.container, target_name: req.target } };
            }
        }

        if (/\b(send|email|mail)\b/i.test(t)
            && /\b(doc|docs|document|documents|invoice|packing|paperwork)\b/i.test(t)
            && !/\b(bol|bill of lading|quote|pricelist|price list|proforma)\b/i.test(t)
            && (m = ctx.text.match(/\b([A-Z]{4}\s?\d{7})\b/i))) {
            return {
                intent: 'send_shipment_docs', resolvedBy: 'policy',
                data: { container_no: m[1].replace(/\s+/g, '').toUpperCase() },
            };
        }
...
send_shipment_docs, generate_sale_invoice, ask_data, ask_text, log_bug, show_bugs, close_bug,
...
const SAFE_ACTIONS = new Set([
    ...
    // Reads a customer's own email, builds a proforma from what it actually
    // says, and asks before sending. Safe for the AI to choose: it stops and
    // asks whenever the email doesn't state a price, and never sends without
    // an explicit yes. See actions.startProformaFromEmail.
    'generate_proforma',
    // Emails a container's ALREADY-GENERATED invoice and packing list to the
    // customer. Safe for the AI to choose: it only ever attaches documents
    // that already exist on disk — it never builds one — and it stages a
    // yes/no confirmation showing the actual address before anything is sent.
    // See actions.sendShipmentDocsForConfirm.
    'send_shipment_docs',
    // Finds the supplier's latest bill and its sale, ASKS for anything
    // missing and for the invoice number, and asks again before saving,
    // generating or emailing. See actions.startSaleInvoice.
    'generate_sale_invoice',
    // Answers a question about her Edge Metals ledgers by running ONE
    // read-only SQL query over a mirror of them. Safe for the AI to choose:
    // it cannot write (helpers/data/sqlGuard), it shows the query and the
    // rows it used, and every figure it says comes from the query result
    // rather than from the model. See actions.askLedger.
    'ask_data',
    // Searches what is already on this server — assessed mail, sent mail,
    // printed invoices, saved documents, quote replies, her facts — and
    // answers ONLY from what it retrieved, citing each source. Read-only, and
    // it says so when the answer is not in what it found. See actions.askText.
    'ask_text',
    // Bugzilla (2026-09-24). Filing and listing are safe for the AI to choose:
    // a report is cheap and reversible, and the list is read-only. close_bug
    // is here too because it only STAGES a yes/no — nothing changes status
    // until she says yes. See actions.logBug / showBugs / closeBugForConfirm.
    'log_bug', 'show_bugs', 'close_bug',
    // Reads her own sent mail and derives a style profile. Read-only, sends
    // nothing, and the profile is scrubbed of business content before it is
    // stored — see helpers/writingStyle.js.
    'learn_writing_style', 'show_writing_style',
    // Re-reads mail already assessed — read-only, sends nothing.
    'rescan_mail',
    ...
    'lookup_address',
    // Recurring reminders (2026-08-22). set_reminder writes a scheduled task,
    // so it isn't read-only like lookup_address — but it's still safe to let
    // the AI classify, because the only thing it can do is schedule a MESSAGE
    // she explicitly asked to be sent, it confirms exactly what it scheduled
    // and when, and it's reversible in one message ("cancel reminder <id>").
    // No booking state, no email, no money.
    'set_reminder', 'show_reminders', 'cancel_reminder',
    // Plain "send this to X" over WhatsApp (2026-08-22). Safe for the AI to
    // classify even though it sends outward, because — unlike draft_email —
    // the AI does NOT author the content: the manager supplies both the exact
    // text and the exact recipient, and the handler refuses to guess when the
    // name is ambiguous. See actions.sendMessageTo's comment.
    'send_message',
    // Receivables (2026-08-22). show_receivables/show_orphan_payments are
    // read-only. record_payment writes, but it's a bookkeeping entry about
    // money ALREADY received — it sends nothing, moves nothing, and the
    // handler refuses to guess which invoice when the reference is ambiguous
...
async function route(decision, ctx, sendMessage) {
    ...
    switch (decision.intent) {
        ...
        // Recurring reminders (2026-08-22). `fact` carries the timing phrase
        // ("every morning", "every day at 8am") per the prompt; falling back
        // to the raw message means a reminder still gets a sane schedule even
        // if the AI didn't isolate the timing words.
        case 'send_message':           return actions.sendMessageTo(chatId, { target: d.target_name, message: d.note || d.email_details });
        ...
        case 'send_shipment_docs':      return actions.sendShipmentDocsForConfirm(chatId, d.container_no, ctx.text);
        case 'generate_sale_invoice':   return actions.startSaleInvoice(chatId, { target: d.target_name, container: d.container_no || d.container });
        case 'ask_data':               return actions.askLedger(chatId, d.question || ctx.text);
        case 'ask_text':               return actions.askText(chatId, d.question || ctx.text);
        case 'log_bug':                return actions.logBug(chatId, { title: d.note || ctx.text, detail: d.details || null, area: d.location || null });
        case 'show_bugs':              return actions.showBugs(chatId, d.filter || 'open');
        case 'close_bug':              return actions.closeBugForConfirm(chatId, { ref: d.note || d.target_name, note: d.fact || null, status: d.filter === 'verified' ? 'verified' : 'fixed' });
        case 'sale_invoice_answer':     return actions.saleInvoiceAnswer(chatId, ctx.pendingAction, d.answer_text || ctx.text);
        case 'bug_close_yes':          return actions.applyBugClose(chatId, ctx.pendingAction);
        case 'generate_proforma':      return actions.startProformaFromEmail(chatId, d.target_name);
        case 'learn_writing_style':    return actions.learnWritingStyle(chatId);
...
function pendingFullReminder(p) {
    ...
    if (p.type === 'await_sale_invoice' && p.question) {
        return `(Still waiting on the invoice${p.container ? ` for ${p.container}` : ''}: ${p.question} — or "cancel".)`;
    }
    ...
    if (decision.needsAI) {
        ...
        decision = {
            ...
            data      : {
                ...
                // ask_data carries her question verbatim (2026-09-23). Without
                // it here the action fires with an empty question and answers
                // nothing — the field-forwarding bug this list exists to stop.
